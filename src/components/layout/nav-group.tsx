...
export function NavGroup({ title, items }: NavGroupType) {
  ...
  const isUrlActive = (url: string) => {
    ...
    if (
      cleanUrl === '/admin/verification' &&
      cleanCurrent === '/admin/verification'
    )
      return true
    ...
    <SidebarGroup>
      ...
      <SidebarMenu>
        {items.map((item) => {
          ...
          const parentBadge =
            item.title === 'Verification' && queue && queue.length > 0
              ? queue.length
                  ...
                  <CollapsibleTrigger asChild>
                    <SidebarMenuButton tooltip={item.title}>
                      <div className='flex items-center gap-2'>
                        ...
                        {parentBadge && (
                          <span className='ml-2 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-white shadow-sm'>
                            {parentBadge}
                          </span>
                        )}
                      </div>
          ...
          return (
            <SidebarMenuItem key={item.title}>
              <SidebarMenuButton
                ...
                tooltip={item.title}
              >
                {/* Removed justify-between from Link, added items-center to div */}
                <Link
                  to={item.url ?? ''}
                  className='flex w-full items-center pr-2'
                >
                  <div className='flex items-center gap-2'>
                    {item.icon && <item.icon className='!size-5' />}
                    <span>{item.title}</span>

                    {/* Render Badge (Specifically handles Whitelisting here) - MOVED INSIDE THE DIV */}
