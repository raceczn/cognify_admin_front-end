...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      'fixed top-4 right-4 z-[100] flex w-[380px] max-w-full flex-col gap-2',
      className
    )}
    {...props}
  />
...
const toastVariants = cva(
  'group pointer-events-auto relative flex w-full items-center justify-between space-x-2 overflow-hidden rounded-md border p-4 pr-6 shadow-xl transition-all',
  {
    variants: {
      variant: {
        default:
          'border-blue-600 bg-blue-100 text-blue-900 dark:bg-blue-900 dark:text-blue-200',
        success:
          'border-green-600 bg-green-100 text-green-900 dark:bg-green-900 dark:text-green-200',
        destructive:
